...
function buildQuiz(qID, qText, qAnswers, qLength, qTypeCounter){
  ...
  // build HTML in parts
  var quizTop = '<section class="quizBody"><h2 class="quizQuestion"><span class="questionNum">' + qID + '/' + qLength + '</span> ' + qText + '</h2><span class="quizAlert">Этот вопрос остался без ответа!</span><div class="row">';
  var quizBottom = '</div><footer><div class="row"><div class="col-sm-offset-3 col-sm-3 col-xs-6"><button class="btn btn-lg btn-default btnBack inactive"><i class="fa fa-chevron-left" aria-hidden="true"></i> Вернуться</button></div><div class="col-sm-3 col-xs-6"><button class="btn btn-lg btn-default btnNext">Далее <i class="fa fa-chevron-right" aria-hidden="true"></i></button></div></div></footer></section>';
  // assemble HTML parts
